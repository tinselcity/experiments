        ...
        */
        ...
        if (sf->tokenvec[left].type == TYPE_NUMBER &&
            ...
            continue;
                   ...
                   sf->tokenvec[left+1].type == TYPE_OPERATOR &&
                   strneq(sf->tokenvec[left+1].val, "::", sizeof("::")) &&
...
int libinjection_sqli_blacklist(struct libinjection_sqli_state* sql_state)
{
    ...
    char ch;
    size_t i;
    size_t len = strnlen(sql_state->fingerprint, sizeof(sql_state->fingerprint));
    int patmatch;
...
int libinjection_sqli_not_whitelist(struct libinjection_sqli_state* sql_state)
{
     ...
     */
    char ch;
    size_t tlen = strnlen(sql_state->fingerprint, sizeof(sql_state->fingerprint));
    ...
    if (tlen > 1 && sql_state->fingerprint[tlen-1] == TYPE_COMMENT) {
        ...
        if (my_memmem(sql_state->s, sql_state->slen,
                      "sp_password", strnlen("sp_password", sizeof("sp_password")))) {
            sql_state->reason = __LINE__;
            return TRUE;
        }
    }
    ...
    case 3:{
        ...
        if (strneq(sql_state->fingerprint, "sos", sizeof("sos"))
            || strneq(sql_state->fingerprint, "s&s", sizeof("s&s"))) {
                ...
                sql_state->reason = __LINE__;
                return FALSE;
        } else if (strneq(sql_state->fingerprint, "s&n", sizeof("s&n")) ||
                   strneq(sql_state->fingerprint, "n&1", sizeof("n&1")) ||
                   strneq(sql_state->fingerprint, "1&1", sizeof("1&1")) ||
                   strneq(sql_state->fingerprint, "1&v", sizeof("1&v")) ||
                   strneq(sql_state->fingerprint, "1&s", sizeof("1&s"))) {
...
int libinjection_is_sqli(struct libinjection_sqli_state * sql_state)
{
    ...
    if (sql_state->lookup(sql_state, LOOKUP_FINGERPRINT,
                          sql_state->fingerprint, strnlen(sql_state->fingerprint, sizeof(sql_state->fingerprint)))) {
        return TRUE;
...
int libinjection_sqli(const char* s, size_t slen, char fingerprint[])
{
    ...
    if (issqli) {
        strncpy(fingerprint, state.fingerprint, sizeof(state.fingerprint));
    } else {
        fingerprint[0] = '\0';
    }
    return issqli;
